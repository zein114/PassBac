...
export function Navbar({ locale }: { locale: string }) {
    ...
    return (
        <nav className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-lg border-b border-gray-200/80 shadow-sm">
            ...
            {/* Mobile nav */}
            <div className="lg:hidden border-t border-gray-100 bg-white flex overflow-x-auto no-scrollbar scroll-smooth">
                {links.map(({ href, label, icon: Icon }) => {
                    const isActive = pathname === href || (href !== '/dashboard' && pathname.startsWith(href));
                    return (
                        <Link
                            key={href}
                            href={href}
                            className={`flex flex-col items-center justify-center gap-1 px-5 py-2 text-[10px] font-bold uppercase tracking-tight flex-shrink-0 transition-colors ${isActive ? 'text-indigo-600 border-t-2 border-indigo-600 -mt-[1px]' : 'text-gray-400 font-medium'
                                }`}
                        >
